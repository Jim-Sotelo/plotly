...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultSample= samples.filter(sampleID => sampleID.id== sample);
    // console.log(resultSample)
    // Deliverable 3: 1. Create a variable that filters the metadata array for the object with the desired sample number.
    var metaFilter = data.metadata.filter(dID => dID.id == sample);
    // Deliverable 1: 5. Create a variable that holds the first sample in the array.
    var result = resultSample[0];
    // Deliverable 3: 2. Create a variable that holds the first sample in the metadata array.
    var firstSample = metaFilter[0];
    ...
    var washFrequeny = parseFloat(firstSample.wfreq);
